<template>
  <div class="monitorView">
    ...
    <section class="main_content">
      <div class="content_bottom" v-for="(chart,index) in chartListOther" :key="index">
        <!-- 图表 -->
        <div class="tipInfo">
          <div>
            <h4 class="fl">{{chart.title}}</h4>
            <button v-if="power=='rw'" style="font-size: 0.8rem" class="fr clickable" @click="deleteChart(chart._id,chartListOther,index)" title="删除"><i class="icon-delete"></i></button>
      ...
      <div class="content_bottom" v-for="(val,index) in chartListFilterTable">
        ...
        <div class="tipInfo">
          <div>
            <h4 class="fl">{{chartListTable[index] && chartListTable[index].title ? chartListTable[index].title : ''}}</h4>
            <button v-if="power=='rw'" style="font-size: 0.8rem" class="fr clickable" @click="deleteChart(chartListTable[index]._id,chartListFilterTable,index,'表格')" title="删除"><i class="icon-delete"></i></button>
            <button v-if="power=='rw'" class="fr clickable" title="添加到概览" @click="addmychart(chartListTable[index])"><i class="icon-pushToHome"></i></button>
            <button style="font-size: 0.8rem" class="fr clickable" title="刷新" @click="tableDataRefresh(index)"><i class="icon-refresh" style="font-size: 0.8rem"></i></button>
            <button v-if="power=='rw'" class="fr clickable" title="设置" @click="editView(chartListTable[index])"><i class="icon-setting"></i></button>
          </div>
        </div>
          ...
          <div class="main_box">
            ...
            <div class="baseInfo_box" v-show="monitoringInfo.navIndex === 0">
              ...
              <ul class="chart_type_box clearfix">
                <li class="fl clickable" v-if="chartVal=='折线图'||chartVal=='柱图'||chartVal=='散点图'||chartVal=='表格'" :class="{'active':editChart.nomalinfo.type===chartVal,'disable':chartVal!='折线图'&&chartVal!='柱图'&&chartVal!='散点图'&&chartVal!='表格'}" @click="editChart.nomalinfo.type=chartVal" v-for="(chartVal,chartIndex) in monitoringInfo.chartType">{{chartVal}}</li>
                <li class="fl" v-if="chartVal!='折线图'&&chartVal!='柱图'&&chartVal!='散点图'&&chartVal!='表格'" :class="{'active':editChart.nomalinfo.type===chartVal,'disable':chartVal!='折线图'&&chartVal!='柱图'&&chartVal!='散点图'&&chartVal!='表格'}" v-for="(chartVal,chartIndex) in monitoringInfo.chartType">{{chartVal}}</li>
              </ul>
            ...
            <div class="target_box" v-show="monitoringInfo.navIndex === 1">
              <div class="target_content_box" v-for="(indexitem,index) in editChart.pfm_index">
                ...
                <div class="one clearfix">
                  ...
                  <el-select v-model="indexitem.zbname" @change="zbnameChange(indexitem,index)" class="fl target_input ml" filterable remote reserve-keyword placeholder="请输入" :remote-method="getzbnameList" :loading="loading">
                ...
                <div class="four">
                  ...
                  <!--<el-checkbox-group v-model="mestypechecks">-->
                    <!--<el-checkbox  :label="'是否关联告警'" class="tzfs_check clickable"></el-checkbox>-->
                  <!--</el-checkbox-group>-->


                  <!--<el-select  v-model="" class="fr" placeholder="请选择用户">-->
                ...
                <div class="three clearfix">
                  ...
                  <div class="clearfix fr check_box">

                    <!--<el-checkbox-group v-model="indexitem.isRelateAlert" class="fl">-->
                      <!--<el-checkbox  :label="'是否关联告警'" class="tzfs_check clickable"></el-checkbox>-->
                    <!--</el-checkbox-group>-->


                    <!--<el-select  v-model="indexitem.relateAlertValue" v-show="true" class="fl" placeholder="请选择用户">-->
                      <!--<el-option v-for="(el,z1) in indexitem.relateAlertArr"  :label="el.relateAlertName" :value="el.relateAlertName">-->
                      <!--</el-option>-->
                    <!--</el-select>-->
            ...
            <div class="charstyle_box" v-show="monitoringInfo.navIndex === 2">
              ...
              <div class="bottom_box">
                ...
                <!--<el-checkbox-group v-model="editChart.isRelateAlert" class="fl">-->
                  <!--<el-checkbox  :label="'是否关联告警'" class="tzfs_check clickable"></el-checkbox>-->
                <!--</el-checkbox-group>-->

                <div class="fl four">阈值线</div>

                <!--indexitem.isRelateAlert-->

                <el-select  v-model="editChart.relateAlertValue" v-show="true" class="fl" placeholder="请选择关联告警">
                  <el-option :label="'无'" :value="'无'">
                  </el-option>
                  <el-option v-for="(el,z1) in relateAlertArr"  :label="el.relateAlertName" :value="el.value">
                  </el-option>
                </el-select>
              </div>
            </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      zbnameChange: function(item,index) {
        //指标名更改(element组件 主动改数据也触发change,只要数据更改就触发)
        console.info('指标名更改', item)
        if (item.zbname != "") {
          this.gettagkey(item)
          if(index === 0) {
            this.relateAlertFun(item);//获取关联警告数组
          }
        }
      ...
      relateAlertFun:function (item) {
          http.get(urls.StrateRelated, {metric:item.zbname}).then(res => {
              this.relateAlertArr = res.data;
            },
            err => {
            }
          )
      ...
      editView: function(chart) {
        ...
        if (chart_option) {
          ...
          chart.isRelateAlert = chart.isRelateAlert ? true : false; //如果服务器没返回值
          chart.relateAlertValue = chart.relateAlertValue ? chart.relateAlertValue : '无'; //如果服务器没返回值
          this.editChart.isRelateAlert = chart.isRelateAlert;
          //this.editChart.isRelateAlert = chart.relateAlertValue && chart.relateAlertValue.length > 0 ? true : false;
          this.editChart.relateAlertValue = chart.relateAlertValue;
          // console.info('指标数组', this.editChart)
          let gettagvalues = [] //获取标签值请求数组
          chart_option.xnzbs.forEach((item,index) => {
            //遍历指标数组
            if(index === 0) {
              this.relateAlertFun(item); //根据第一个指标名获取关联告警值数组
            }
      ...
      delxnzb: function(index) {
        ...
        if(this.editChart.pfm_index && this.editChart.pfm_index.length == 0) {
          this.relateAlertArr = [];
        }else if(index == 0 && this.editChart.pfm_index[0].zbname){
          this.relateAlertFun(this.editChart.pfm_index[0]);
        }
      },
      ...
      addchart: function() {
        ...
        this.editChart.isRelateAlert = this.editChart.relateAlertValue && this.editChart.relateAlertValue.length > 0 &&  this.editChart.relateAlertValue!== '无' ? true : false;

        //添加图表
        let newoption = _.cloneDeep(this.editChart)  //新图表数据 （对象深拷贝）（发送前处理掉数据库不要的数据）
        if(this.editChart.relateAlertValue === '无') {
          newoption.relateAlertValue = '';
        }
      ...
      deleteChart(id,dataList,index,type){
        //删除图表
        this.$confirm('是否删除该条数据?', '提示', {
          ...
          http.get(urls.removechart, {id:id}).then(res => {
              if(type === '表格'){
                this.chartListTable.splice(index,1);
      ...
      tableDataRefresh(index){ //刷新表格数据
        const arr = [...this.chartListFilterTable];
        const item = this.chartListTable[index];
        const par = {metrics:[]};
        item.xnzbs.forEach((item2) => {
          if(item2.tag && item2.tag.length > 0){
            item2.tag.forEach((item3) => {
        ...
        this.getTableFunc(par).then((value) => {
          arr[index] = value;
          this.chartListFilterTable = arr;
        })
